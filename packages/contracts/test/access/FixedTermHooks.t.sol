...
contract FixedTermHooksTest is BaseAccessControlsTest {
  ...
  function test_onCreateMarket_InvalidFixedTerm() external {
    ...
    // End time in the past still reverts.
    vm.expectRevert(FixedTermHooks.InvalidFixedTerm.selector);
    hooks.onCreateMarket(address(this), address(1), inputs, abi.encode(0));
    // sellbonds.now fork: the prior 365-day cap is removed. Previously a term
    // of 366 days reverted with InvalidFixedTerm; it should now succeed.
    // This assertion is replaced by test_onCreateMarket_LongTermsAllowed below.
  }

  // sellbonds.now fork: prove that terms previously rejected by the cap
  // are now accepted. 10-year term must succeed; uint32.max - block.timestamp
  // must also succeed (effectively the new ceiling, set by uint32 itself).
  function test_onCreateMarket_LongTermsAllowed() external {
    DeployMarketInputs memory inputs;
    ...
    // 10 years — would have reverted under the upstream 365-day cap.
    hooks.onCreateMarket(
      address(this),
      address(0xA1),
      inputs,
      abi.encode(block.timestamp + 3650 days)
    );

    // Right at the uint32 boundary (the absolute hardware ceiling).
    hooks.onCreateMarket(
      address(this),
      address(0xA2),
      inputs,
  ...
  function test_getParameterConstraints() external view {
    ...
    // sellbonds.now fork: penalty APR cap raised from 10_000 to type(uint16).max.
    assertEq(
