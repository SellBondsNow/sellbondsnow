...
contract WildcatArchControllerTest is Test, Prankster {
  ...
  // ---- sellbonds.now fork: permissionless self-registration ----
  ...
  function test_registerSelf() external {
    ...
    vm.prank(borrower);
    archController.registerSelf();

    assertTrue(archController.isRegisteredBorrower(borrower));
  }

  function test_registerSelf_AnyAddress(address agent) external {
    vm.assume(agent != address(0));

    vm.prank(agent);
    archController.registerSelf();

    assertTrue(archController.isRegisteredBorrower(agent));
  }

  function test_registerSelf_BorrowerAlreadyExists() external {
    vm.prank(borrower);
    archController.registerSelf();
  ...
  function test_registerSelf_NoOwnerCheck() external {
    // Explicit proof: a caller that is neither the owner nor pre-approved can
    // still register itself. This is the core invariant of the fork.
    address randomAgent = address(0xA63E47);
    assertFalse(archController.isRegisteredBorrower(randomAgent));
    assertTrue(archController.owner() != randomAgent);

    vm.prank(randomAgent);
    archController.registerSelf();

    assertTrue(archController.isRegisteredBorrower(randomAgent));
  }

  function test_registerSelf_CoexistsWithAdminPath() external {
    // Admin path still works after fork.
    archController.registerBorrower(borrower);
    assertTrue(archController.isRegisteredBorrower(borrower));

    // Self path works independently.
    vm.prank(borrower2);
    archController.registerSelf();
    assertTrue(archController.isRegisteredBorrower(borrower2));

    // Both end up in the same set.
