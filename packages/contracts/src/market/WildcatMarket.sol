  ...
  WildcatMarketWithdrawals
{
   ...
   */
  // sellbonds.now fork: set or change the optional secondary closer. Pass
  // address(0) to clear. Only the borrower may call. The backupCloser cannot
  // change itself; the borrower retains ultimate control.
  function setBackupCloser(address newBackupCloser) external onlyBorrower nonReentrant {
    address old = backupCloser;
    backupCloser = newBackupCloser;
    emit BackupCloserUpdated(old, newBackupCloser);
  }

  // sellbonds.now fork: auth widened from `onlyBorrower` to also accept the
  // optional `backupCloser` address. See WildcatMarketBase.backupCloser.
  function closeMarket() external onlyBorrowerOrBackupCloser nonReentrant sphereXGuardExternal {
    MarketState memory state = _getUpdatedState();
