  ...
  IMarketEventsAndErrors
{
  ...
  // sellbonds.now fork: optional secondary closer address. If set (non-zero),
  // this address may call `closeMarket()` in addition to the borrower. Designed
  // to be a multisig or recovery service so a lost agent key does not leave the
  // market accruing penalty APR indefinitely. address(0) = borrower-only.
  address public backupCloser;
  ...
  // sellbonds.now fork: like onlyBorrower, but also accepts the optional
  // backupCloser address when set. Used by closeMarket() so a designated backup
  // (multisig, recovery service) can settle a market whose borrower key is lost.
  modifier onlyBorrowerOrBackupCloser() {
    if (msg.sender != borrower && msg.sender != backupCloser) {
      revert CallerNotBorrowerOrBackupCloser();
    }
    _;
  }
