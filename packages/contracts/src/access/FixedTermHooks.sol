...
contract FixedTermHooks is BaseAccessControls, MarketConstraintHooks {
  ...
  // sellbonds.now fork: raised from 365 days to type(uint32).max.
  // Decision: bond maturity is agent-chosen; the protocol does not impose a
  // ceiling on how long lenders can lock funds. The duration check itself is
  // preserved so we can re-introduce a cap via config without re-patching.
  uint32 public constant MaximumLoanTerm = type(uint32).max;
