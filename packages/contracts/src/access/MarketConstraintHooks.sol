...
abstract contract MarketConstraintHooks is IHooks {
  ...
  // sellbonds.now fork: raised from 10_000 (100%/yr) to type(uint16).max.
  // Decision: penalty APR is agent-chosen per market; the protocol does not
  // impose a ceiling. Lenders price the risk. The validation hook is preserved
  // so we can re-introduce a cap via config without re-patching.
  uint16 internal constant MaximumDelinquencyFeeBips = type(uint16).max;
