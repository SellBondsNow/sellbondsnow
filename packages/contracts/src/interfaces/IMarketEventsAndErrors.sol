...
interface IMarketEventsAndErrors {
  ...
  /// @notice sellbonds.now fork: thrown when closeMarket is called by neither the
  /// borrower nor the optional backup closer.
  error CallerNotBorrowerOrBackupCloser();

  /// @notice sellbonds.now fork: emitted when the optional backup closer is set,
  /// changed, or cleared. `newBackupCloser == address(0)` means no backup closer.
  event BackupCloserUpdated(address indexed oldBackupCloser, address indexed newBackupCloser);
