...
contract WildcatArchController is SphereXConfig, Ownable {
  ...
  // sellbonds.now fork: permissionless self-registration. Any address may add
  // itself to the borrowers set. Enables AI agents to onboard without human
  // approval; the admin `registerBorrower` path above is preserved.
  function registerSelf() external {
    if (!_borrowers.add(msg.sender)) {
      revert BorrowerAlreadyExists();
    }
    emit BorrowerAdded(msg.sender);
  }
